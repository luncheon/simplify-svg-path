...
class Formatter {
  readonly _multiplier = 10 ** this._precision

  constructor(readonly _precision: number) {}
...
class PathFitter {
  constructor(readonly _points: Point[], readonly _closed?: boolean) {}
  ...
  _fitCubic(segments: Segment[], error: number, first: number, last: number, tan1: Point, tan2: Point) {
    const points = this._points
  ...
  _addCurve(segments: Segment[], curve: Point[]) {
  ...
  _generateBezier(first: number, last: number, uPrime: number[], tan1: Point, tan2: Point) {
  ...
  _reparameterize(first: number, last: number, u: number[], curve: Point[]) {
  ...
  _findRoot(curve: Point[], point: Point, u: number) {
    const curve1 = [],
      curve2 = []
    ...
    const pt = this._evaluate(3, curve, u),
      pt1 = this._evaluate(2, curve1, u),
      pt2 = this._evaluate(1, curve2, u),
  ...
  _evaluate(degree: number, curve: Point[], t: number) {
    // Copy array
  ...
  _chordLengthParameterize(first: number, last: number) {
    const u = [0]
  ...
  _findMaxError(first: number, last: number, curve: Point[], u: number[]) {
  ...
  const addSegment = (segment: Segment, skipLine?: boolean) => {
    ...
    if (first) {
      ...
      if (inX === curX && inY === curY && outX === prevX && outY === prevY) {
        ...
        // c = relative curveto:
        parts.push(
          'c' + f._pair(outX - prevX, outY - prevY) + ' ' + f._pair(inX - prevX, inY - prevY) + ' ' + f._pair(curX - prevX, curY - prevY),
        )
      }
    }
    prevX = curX
